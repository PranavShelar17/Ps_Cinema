...
export default function KidsMoviesSlider(props) {
    ...
    async function fetchKidsMovies() {
      ...
      try {
        const response = await axios.get(
          `https://api.themoviedb.org/3/discover/movie?api_key=${apiKey}&with_genres=10751&sort_by=popularity.desc`
        );
        setKidsMovies(response.data.results);
        console.log(response.data.results);
      } catch (err) {
        console.error("Failed to fetch kids movies", err);
      } finally {
        setLoading(false);
      }
    }
  ...
  function handleKidsImageClick(movie) {
    if (props.onKidsImageClick) props.onKidsImageClick(movie);
  }
  ...
  if (loading) {
    return (
      <div className="text-center my-5 d-flex justify-content-center">
        <RingLoader size={50} color="deepskyblue" />
      </div>
    );
  }
        ...
        {kidsMovies.map((movie) => (
          <div key={movie.id} className="p-2 m-0 movie-card">
            <div className="image-container">
              <img
                onClick={() => handleKidsImageClick(movie)}
